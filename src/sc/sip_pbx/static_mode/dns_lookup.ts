module.exports = function (done) {
  ...
  const result = new SIPpW(process.env.DUT_HOST)
    ...
    .setVariable(
      'from',
      `${process.env.PHONE_E2_USERNAME}@${process.env.SIPPBX_DOMAIN}`
    )
    .setVariable(
      'to',
      `${process.env.PHONE_S1_USERNAME}@${process.env.SIPPBX_DOMAIN}`
    ) // This will trigger a DOMAIN_EGRESS via GW
    .start()
